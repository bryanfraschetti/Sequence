...
app.get("/initiateAuth", (req, res) => {
    ...
    url += "&scope=user-read-currently-playing"
        ...
        + "%20playlist-modify-private" + "%20playlist-modify-public";
    url += "&state=" + state;
...
app.get("/authorizationCallback", (req, res) => {
    ...
    if(!authorizationComplete){//not complete or integrity lost
        res.redirect(entryPoint);
    }

    else{//begin token generation
        const authBody = "grant_type=authorization_code"
            + "&code=" + code
            + "&redirect_uri=" + encodeURI(authCallback)
            + "&client_id=" + clientId;
...
app.post("/RefreshToken", (req, res) => {
    ...
    if(!refresh_token){//missing credentials
        ...
        fetch(
            spotifyTokenUrl + "?grant_type=refresh_token&refresh_token=" + refresh_token, {
            headers: {
                "Authorization": "Basic " + btoa(clientId + ":" + clientSecret),
                "content-type": "application/x-www-form-urlencoded"
            },
